...
exports.parseUIDRanges = function(uidRanges) {
  var fragments = uidRanges.split(',');
  var result = [];
  for (var i=0; i < fragments.length; i++) {
    var range = fragments[i].split(':');
    switch (range.length) {
    case 1:
      var uid = parseInt(fragments[i], 10);
      if (isNaN(uid)) {
        throw new Error('UID ' + fragments[i] + ' is not an integer');
      }
      result.push(uid.toString() === fragments[i] ? uid : fragments[i]);
      break;
    case 2:
      var fromInt = parseInt(range[0], 10), toInt = parseInt(range[1], 10);
      if (isNaN(fromInt) || isNaN(toInt)) {
        throw new Error('UID ' + fragments[i] + ' in range is not an integer');
      }
      if (!(fromInt.toString() === range[0] && toInt.toString() === range[1])) {
        throw new Error('UIDs in range ' + fragments[i] + ' can not be parsed as a number (too large?)');
      }
      for (var j=fromInt; j <= toInt; j++) {
        result.push(j);
      }
      break;
    default:
      throw new Error('UID range may only have two parts');
    }
  }
  return result;
};
