    ...
    if (indata.line[0] === '*') { // Untagged server response
      ...
      if (typeof requests[0].callback === 'function') {
        ...
        if (line[0] === '+') {
          ...
          var m = false;
          if (m = /^A\d+ OK (?:\[(?:APPEND|COPY)UID (\d+) ([\d,:]+)(?: ([\d,:]+))?)/i.exec(line)) {
            if (m[3])
              args.unshift(parsers.parseUIDRanges(m[3])); // COPY and MOVE return a destination UID range, too
            args.unshift(parsers.parseUIDRanges(m[2])); // source or destination UID range
            args.unshift(m[1]); // destination mailbox's UID validity
          } else if (sendBox) // SELECT, EXAMINE, RENAME
...
ImapConnection.prototype.append = function(data, options, cb) {
  ...
  this._send(cmd, function(err, uidvalidity, destuid) {
    if (err || step++ === 2)
      return cb(err, uidvalidity, destuid);
    self.debug&&self.debug('\n==> (' + data.length + ' bytes) \n');
